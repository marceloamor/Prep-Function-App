    ...
    with rjo_sftp_utils.get_rjo_ssh_client() as rjo_ssh:
        with rjo_ssh.open_sftp() as rjo_sftp_client:
            ...
            for file_format in formats_to_fetch:
                ...
                if len(sftp_files) == 0:
                    logging.warning(f"Found no recent enough files with basename {file_format} in RJO SFTP")
                    return []
...
def historical_migration_script():
    with rjo_sftp_utils.get_rjo_ssh_client() as rjo_ssh:
        with rjo_ssh.open_sftp() as rjo_sftp_client:
            ...
            filename_pattern = "UPETRADING_monthlytrans_mtrn_%Y%m%d.csv" # SPECIFY FILE NAME HERE
            for filename in rjo_sftp_client.listdir():
                try:
            ...
            for file in sftp_files:
                try:
                    rjo_sftp_client.get(file[1], f"prep/data_ingestion/temp_assets/{file[1]}")
            ...
            with rjo_sftp_utils.get_upe_ssh_client() as upe_ssh:
                with upe_ssh.open_sftp() as upe_sftp_client:
                    upe_sftp_client.chdir("/rjo_file_backup")
                    for file in sftp_files:
                        upe_sftp_client.put(f"prep/data_ingestion/temp_assets/{file[1]}", file[1])
                        logging.info(f"File {file[1]} has been successfully posted to UPE SFTP")
                        print(f"File {file[1]} has been successfully posted to UPE SFTP")
